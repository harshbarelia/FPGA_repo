...
module udp_payload_framer #(
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      held_valid <= 1'b0; held_data <= '0; held_len <= '0; m_msg_valid <= 1'b0; m_msg_data <= '0; m_msg_len <= '0;
    end else begin
      if (s_msg_valid && s_msg_ready) begin
        held_valid <= 1'b1; held_data <= s_msg_data; held_len <= s_msg_len;
      end
      if (held_valid && (!m_msg_valid || m_msg_ready)) begin
