...
module tb_event_packer;
  ...
  logic [383:0] m_axis_tdata; logic [47:0] m_axis_tkeep; logic m_axis_tvalid, m_axis_tready, m_axis_tlast;

  event_packer #(.BEAT_W(384)) dut(.clk(clk), .rst_n(rst_n), .s_book_event(s_book_event), .s_book_event_valid(s_book_event_valid), .s_book_event_ready(s_book_event_ready), .m_axis_tdata(m_axis_tdata), .m_axis_tkeep(m_axis_tkeep), .m_axis_tvalid(m_axis_tvalid), .m_axis_tready(m_axis_tready), .m_axis_tlast(m_axis_tlast));
  always #5 clk=~clk;
  ...
  initial begin
    ...
    s_book_event_valid=1; @(posedge clk); s_book_event_valid=0; repeat(2) @(posedge clk);
