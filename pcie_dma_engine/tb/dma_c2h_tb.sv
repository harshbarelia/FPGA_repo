...
// dma_c2h_tb.sv
// Card-to-Host smoke test over AXI4-LITE ONLY interfaces: exercises
// the C2H direction explicitly, moving data FROM local BRAM INTO
// host DDR (the reverse of the H2C tests). Descriptor control
// bit3=1 selects C2H in dma_top's desc_out.control[3] mux.
`timescale 1ns/1ps
import dma_pkg::*;
...
module dma_c2h_tb;
    ...
    localparam logic [31:0] RING_BASE = HOST_DDR_BASE + 32'h0000_1000; // ring in host DDR
    localparam logic [31:0] RING_SIZE = 32'd16;

    // C2H: src lives in local BRAM, dst lives in HOST DDR (BRAM->DDR,
    // the reverse of the H2C DDR->BRAM tests).
    localparam logic [31:0] SRC_ADDR  = BRAM_BASE;
    localparam logic [31:0] DST_ADDR  = HOST_DDR_BASE + 32'h0000_5000;
    ...
    initial begin
        wait (rst_n);
        @(posedge clk);
        // Pre-load BRAM source region with known test pattern (BRAM
        // starts zeroed in the BFM, so we must seed it before C2H
        // can move anything meaningful out of it).
        for (int i = 0; i < XFER_LEN; i++)
            u_bfm.bram_mem[SRC_ADDR - BRAM_BASE + i] = i[7:0] ^ 8'hA5;

        // control: IRQ_EN=1 (bit1), DIR=C2H (bit3=1) -> 32'h0000_000A
        write_desc_slot(0, SRC_ADDR, DST_ADDR, XFER_LEN, 32'h0000_000A);
    end
    ...
    initial begin
        ...
        axil_write(12'h004, RING_BASE);
        axil_write(12'h00C, RING_SIZE);
        axil_write(12'h018, 32'h0000_0001); // IRQ_EN
        axil_write(12'h000, 32'h0000_0001); // dma_enable

        axil_write(12'h010, 32'd1); // doorbell: TAIL_PTR=1

        wait (msix_irq === 1'b1);
        $display("[C2H_TB] MSI-X interrupt observed at time %0t - C2H BRAM->DDR transfer complete", $time);

        repeat (5) @(posedge clk);
        for (int i = 0; i < XFER_LEN; i++) begin
            byte src_byte = u_bfm.bram_mem[SRC_ADDR - BRAM_BASE + i];
            byte dst_byte = u_bfm.ddr_mem[DST_ADDR - HOST_DDR_BASE + i];
            if (dst_byte !== src_byte) begin
                $error("[C2H_TB] Data mismatch at byte %0d: ddr_dst=%0h bram_src=%0h", i, dst_byte, src_byte);
            end
        end
        $display("[C2H_TB] C2H BRAM->DDR single-descriptor smoke test PASSED");

        repeat (20) @(posedge clk);
        $finish;
    end

endmodule : dma_c2h_tb
