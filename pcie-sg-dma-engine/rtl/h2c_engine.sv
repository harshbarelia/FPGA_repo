...
// h2c_engine.sv
// Host-to-Card DMA engine over AXI4-LITE (no bursts, one beat at a
// time). For each BEAT_BYTES chunk: issue single-beat AXI4-Lite READ
// from src_addr (HOST DDR), wait for RDATA, then issue single-beat
// AXI4-Lite WRITE of that data to dst_addr (local BRAM), wait for
// BRESP, advance addresses by BEAT_BYTES, decrement remaining
// length. Repeats until done.
import dma_pkg::*;
